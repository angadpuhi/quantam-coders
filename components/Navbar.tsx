...
import { Activity, Languages, LogIn, LogOut, ShieldAlert, User, Stethoscope } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full border-b border-emerald-100 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="hidden lg:flex items-center text-xs font-medium text-slate-600 bg-slate-100 px-2.5 py-1.5 rounded-lg border border-slate-200">
            <Languages className="w-4 h-4 mr-1.5 text-emerald-600" />
            <span>বাংলা | हिन्दी | മലയാളം | ଓଡ଼ിଆ</span>
          </div>

          <Link
            href="/"
            className="text-xs sm:text-sm font-medium text-slate-700 hover:text-emerald-700 px-2.5 py-1.5 rounded-md hover:bg-emerald-50 transition"
          >
            Worker Lookup
          </Link>
          ...
          {status === "loading" ? (
                ...
                <div className="flex flex-col text-left">
                  ...
                  <span
                    className={`text-[10px] font-bold ${
                      session.user.role === "ADMIN" ? "text-amber-700" : "text-emerald-700"
                    }`}
                  >
                    {session.user.role}
                  </span>
            ...
            <Link
              href="/login"
              className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-medium px-3.5 py-1.5 rounded-lg shadow-xs transition"
            >
