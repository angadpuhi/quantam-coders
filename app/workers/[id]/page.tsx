    ...
    return (
      <div className="max-w-2xl mx-auto py-12 text-center">
        <div className="bg-white border border-red-200 rounded-houseboat p-8 shadow-sm">
          ...
          <h2 className="text-xl font-bold text-slate-900">Worker Profile Not Found</h2>
          <p className="text-sm text-slate-600 mt-2">{error || "The requested worker record does not exist."}</p>
          <div className="mt-6">
            <Link
              href="/registry"
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>{showAddForm ? "Hide Clinical Form" : "Add Clinical Record"}</span>
        </button>
      </div>
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/30 border border-kerala-gold-400/40 text-xs font-semibold text-kerala-gold-200 mb-3 backdrop-blur-xs">
                ...
                <span>Verified Athidhi Swasthya Profile</span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-emerald-100 mt-2">
                ...
                <span>•</span>
                <span>
                  Origin: <strong>{workerData.homeState}</strong>
                </span>
                {workerData.phone && (
                  <>
                    <span>•</span>
                    <span>Phone: {workerData.phone}</span>
                  </>
                )}
              </div>
            </div>
          ...
          {workerData.currentAddress && (
            <div className="mt-5 pt-4 border-t border-white/15 text-xs text-emerald-200 flex items-center gap-2">
              ...
              <span>Current Worksite / Residential Address: {workerData.currentAddress}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="bg-white border border-kerala-coir-200 rounded-houseboat p-6 sm:p-8 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between gap-2 border-b border-kerala-coir-100 pb-4 mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Clinical Consultation History</h2>
                <p className="text-xs text-slate-500">
                  Doctor visits across Primary & Community Health Centres in Kerala
                </p>
              </div>
            </div>
            <span className="text-xs font-semibold bg-kerala-green-50 text-kerala-green-900 px-2.5 py-1 rounded-full">
              {workerData.visits?.length || 0} Visits Logged
            </span>
          </div>
          ...
          {workerData.visits && workerData.visits.length > 0 ? (
              ...
              {workerData.visits.map((visit: any) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-700 leading-relaxed">
                    <strong className="text-slate-900">Doctor Notes / Symptoms:</strong> {visit.notes}
                  </p>
            ...
            <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-2xl">
              ...
              <p className="text-xs font-medium">No clinical visits recorded yet for this worker.</p>
              <button
                type="button"
        ...
        <div className="bg-white border border-kerala-coir-200 rounded-houseboat p-6 sm:p-8 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between gap-2 border-b border-kerala-coir-100 pb-4 mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Diagnostic & Health Screenings</h2>
                <p className="text-xs text-slate-500">
                  Occupational health checks, infectious disease screenings (TB, Malaria) & vitals
                </p>
              </div>
            </div>
            <span className="text-xs font-semibold bg-kerala-blue-50 text-kerala-blue-900 px-2.5 py-1 rounded-full">
              {workerData.screenings?.length || 0} Screenings
            </span>
          </div>
          ...
          {workerData.screenings && workerData.screenings.length > 0 ? (
            ...
            <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-2xl">
              ...
              <p className="text-xs font-medium">No diagnostic screenings logged yet.</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-kerala-coir-200 rounded-houseboat p-6 sm:p-8 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between gap-2 border-b border-kerala-coir-100 pb-4 mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Treatment & Medication History</h2>
                <p className="text-xs text-slate-500">
                  Prescription logs and continuous occupational health care regimens
                </p>
              </div>
            </div>
            <span className="text-xs font-semibold bg-kerala-gold-50 text-kerala-gold-900 px-2.5 py-1 rounded-full">
              {workerData.treatments?.length || 0} Treatments
            </span>
          </div>
          ...
          {workerData.treatments && workerData.treatments.length > 0 ? (
              ...
              {workerData.treatments.map((tr: any) => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500 shrink-0">
                    <span>Prescribed: {formatDate(tr.date)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-2xl">
              ...
              <p className="text-xs font-medium">No treatments prescribed yet.</p>
            </div>
          )}
        </div>
      </div>
